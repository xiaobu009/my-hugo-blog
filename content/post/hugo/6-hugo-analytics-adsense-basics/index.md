...
3. 点 **Sitemap**，右上角输入框里填入站点地图地址，点提交。填写格式取决于你当时选的资源类型：
   - **网域属性**（验证时填的是 `yourblog.com`，不带 `https://`）→ 填完整 URL：`https://yourblog.com/sitemap.xml`
   - **网址前缀属性**（验证时填的是 `https://yourblog.com/`）→ 填相对路径：`sitemap.xml`
...
这一步很多人会在内容还没攒够的时候就急着申请，结果反复被拒还摸不清原因。这里先把现在的几个现实情况说清楚。
